...
Validasi model pada skenario operasional dilakukan melalui pengujian \textit{benchmark} pada video uji berdurasi 3.000 \textit{frame} (resolusi $1280 \times 720$, 25 FPS) dengan \textit{confidence threshold} 0,25 dan IoU \textit{threshold} 0,7. Hasil distribusi deteksi per kelas yang tercatat selama \textit{benchmark} disajikan pada Tabel~\ref{tab:distribusi-deteksi}. Model mendeteksi total 11.167 objek dengan rerata 3,72 objek per \textit{frame}, terdiri dari 4.399 deteksi \texttt{DRIVER\_HELMET} (1,47/\textit{frame}), 1.711 deteksi \texttt{DRIVER\_NO\_HELMET} (0,57/\textit{frame}), dan 5.057 deteksi \texttt{MOTORCYCLE} (1,69/\textit{frame}).
...
\hline
\texttt{DRIVER\_HELMET}     & 4.399 & 1,47 \\
\texttt{DRIVER\_NO\_HELMET} & 1.711 & 0,57 \\
\texttt{MOTORCYCLE}         & 5.057 & 1,69 \\
\hline
\textbf{Total}              & 11.167 & 3,72 \\
\hline
\end{tabular}
\end{table}

Rasio antara kelas \texttt{DRIVER\_HELMET} dan \texttt{DRIVER\_NO\_HELMET} sebesar 2,57:1 mengindikasikan bahwa mayoritas pengendara pada lokasi uji telah mematuhi kewajiban penggunaan helm. Meskipun demikian, proporsi pelanggaran yang mencapai 28,0\% dari total deteksi pengendara merupakan angka yang cukup signifikan dan memperkuat justifikasi kebutuhan akan sistem pemantauan otomatis berbasis \textit{computer vision} \cite{jia2021real}. Penggunaan \textit{confidence threshold} operasional yang lebih rendah (0,25) dibandingkan ambang pelatihan (0,5) dirancang untuk mengakomodasi deteksi berkonfidensialitas rendah yang diperlukan oleh mekanisme asosiasi dua tahap ByteTrack \cite{zhang2022bytetrack}, sehingga \textit{track} yang sempat terdeteksi dengan konfidensialitas rendah tetap dapat diasosiasikan dan dilacak secara persisten. Rerata 3,72 objek per \textit{frame} merepresentasikan kepadatan lalu lintas sedang yang konsisten dengan karakteristik gerbang kampus, suatu kondisi yang relatif \textit{favorable} bagi model deteksi karena meminimalkan oklusi antar-objek. Dalam konteks perbandingan dengan studi sejenis pada lingkungan lalu lintas perkotaan yang lebih padat \cite{kumar2021helmet}, tingkat deteksi per \textit{frame} pada penelitian ini tergolong moderat dan memungkinkan pelacakan objek yang lebih stabil.
...
Evaluasi stabilitas pelacakan ByteTrack pada video uji 3.000 \textit{frame} menghasilkan 134 identitas unik (\textit{track ID}) yang distingtif, dengan rerata 3,37 objek aktif per \textit{frame} dan kapasitas maksimal 11 objek simultan, sebagaimana dirangkum pada Tabel~\ref{tab:statistik-tracking}. Konfigurasi pelacakan menggunakan \textit{track activation threshold} 0,4, \textit{minimum matching threshold} 0,85, dan \textit{lost track buffer} 120 \textit{frame}. Visualisasi hasil pelacakan dengan identitas persisten dan asosiasi spasial \textit{rider}--\textit{motorcycle} ditampilkan pada Gambar~\ref{fig:tracking-vis}.
...
\textbf{Metrik} & \textbf{Nilai} \\
\hline
Total \textit{track ID} unik            & 134 \\
Rata-rata objek aktif per \textit{frame} & 3,37 \\
Maksimum objek simultan                  & 11 \\
\textit{Lost track buffer}              & 120 \textit{frame} ($\approx$4,8 detik) \\
\textit{Minimum matching threshold}      & 0,85 \\
\textit{Track activation threshold}      & 0,4 \\
\hline
\end{tabular}
\end{table}
...
Keberhasilan ByteTrack dalam mempertahankan 134 identitas unik tanpa fragmentasi signifikan dapat diatribusikan pada mekanisme asosiasi dua tahap (\textit{two-stage association}) yang menjadi keunggulan fundamental algoritma ini \cite{zhang2022bytetrack}: deteksi berkonfidensialitas tinggi diasosiasikan terlebih dahulu melalui pencocokan berbasis IoU, kemudian deteksi berkonfidensialitas rendah dimanfaatkan untuk merecoveri \textit{track} yang hilang akibat oklusi sesaat. Konfigurasi \textit{track activation threshold} 0,4 memungkinkan ByteTrack memulai pelacakan pada deteksi dengan konfidensialitas sedang, sementara \textit{matching threshold} 0,85 yang relatif tinggi menghasilkan pencocokan yang lebih permisif---biaya asosiasi dihitung sebagai $1 - \text{IoU} \times \text{confidence}$, sehingga ambang 0,85 berarti asosiasi diterima jika $\text{IoU} \times \text{confidence} > 0,15$---mengurangi fragmentasi identitas pada skenario oklusi parsial. Kapabilitas penanganan hingga 11 objek simultan mendemonstrasikan skalabilitas yang memadai untuk kondisi kepadatan puncak pada gerbang kampus. Konfigurasi \textit{lost track buffer} sebesar 120 \textit{frame}---ekuivalen dengan $\approx$4,8 detik pada laju pemrosesan efektif---memungkinkan persistensi identitas pengendara selama oklusi temporal yang cukup panjang, suatu parameter yang dioptimasi berdasarkan rerata durasi lintasan kendaraan melewati \textit{field of view} kamera. Stabilitas pelacakan ini merupakan prasyarat esensial bagi mekanisme konfirmasi temporal N-\textit{frame} yang dibahas pada subbab berikutnya, karena akurasi identifikasi pelanggaran bergantung langsung pada konsistensi asosiasi identitas objek lintas-\textit{frame} \cite{dendorfer2021motchallenge}.
...
Eksperimen komparatif dengan tiga variasi \textit{threshold} temporal ($N = 1, 3, 5$) pada 3.000 \textit{frame} video uji menghasilkan gradasi yang jelas antara sensitivitas dan reliabilitas, sebagaimana dirangkum pada Tabel~\ref{tab:threshold-comparison}. Konfigurasi $N = 1$ mengonfirmasi 59 pelanggaran dengan \textit{confidence} rata-rata 0,736; konfigurasi $N = 3$ mengonfirmasi 57 pelanggaran dengan \textit{confidence} 0,763; dan konfigurasi $N = 5$ mengonfirmasi 56 pelanggaran dengan \textit{confidence} 0,779. Perbandingan visual antar-konfigurasi diilustrasikan pada Gambar~\ref{fig:threshold-comparison}.
...
\hline
1 & 59 & 0,736 & Tercepat, rentan \textit{noise} \\
3 & 57 & 0,763 & Optimal \\
5 & 56 & 0,779 & \textit{Confidence} tertinggi, \textit{delay} lebih besar \\
\hline
\end{tabular}
\end{table}
...
Konfigurasi $N = 3$ terpilih sebagai nilai optimal berdasarkan analisis keseimbangan antara sensitivitas dan spesifisitas. Investigasi mendalam terhadap dua deteksi tambahan pada $N = 1$ mengonfirmasi bahwa keduanya merupakan \textit{false violation} yang dipicu oleh kesalahan klasifikasi sesaat pada satu \textit{frame}, mengindikasikan kerentanan pendekatan tanpa filter temporal terhadap \textit{noise} deteksi \cite{han2020mining}. Sebaliknya, konfigurasi $N = 5$ kehilangan 1 pelanggaran yang tervalidasi secara visual karena durasi lintasan objek tidak mencukupi ambang batas 5 \textit{frame}. Konfigurasi $N = 3$ meningkatkan \textit{confidence} rata-rata sebesar 3,7\% (dari 0,736 menjadi 0,763) dibandingkan $N = 1$ sambil mengeliminasi seluruh \textit{false positive}, dan mempertahankan 1 pelanggaran valid tambahan dibandingkan $N = 5$ dengan penambahan \textit{delay} konfirmasi yang marginal ($\approx$390 ms pada 7,63 FPS). Mekanisme ini, dilengkapi dengan \textit{patience-based decay} 5 \textit{frame} dan filter spasial IoA $\geq$ 0,3 terhadap \textit{bounding box} sepeda motor, secara empiris mengonfirmasi efektivitas pendekatan \textit{multi-frame temporal consistency} dalam menyaring \textit{noise} deteksi pada aplikasi surveilans \cite{pareek2021survey}.
...
Pengukuran \textit{throughput} selama pemrosesan 3.000 \textit{frame} pada prosesor Intel Core i5-5200U tanpa akselerasi GPU menghasilkan FPS rata-rata 7,63 (median 7,13), dengan rentang 1,26--9,67 FPS dan standar deviasi 1,28 FPS, sebagaimana dirinci pada Tabel~\ref{tab:throughput}. Total waktu pemrosesan tercatat 413,5 detik. Distribusi FPS sepanjang durasi pemrosesan divisualisasikan pada Gambar~\ref{fig:fps-over-time}.
...
FPS minimum            & 1,26 \\
FPS maksimum           & 9,67 \\
FPS persentil ke-5     & 6,02 \\
FPS persentil ke-95    & 9,33 \\
Standar deviasi FPS    & 1,28 \\
Total waktu pemrosesan & 413,5 detik \\
\hline
\end{tabular}
\end{table}
...
Capaian \textit{throughput} 7,63 FPS pada CPU merupakan konsekuensi yang diharapkan mengingat dominasi \textit{latency} inferensi YOLOv8 pada perangkat tanpa akselerasi GPU \cite{jocher2023ultralytics}. Penggunaan \textit{confidence threshold} yang rendah (0,25) menghasilkan lebih banyak deteksi per \textit{frame} (3,72), meningkatkan beban komputasi pada tahap pelacakan dan \textit{non-maximum suppression}. Meskipun lebih rendah dari laju video asli (25 FPS), nilai ini tetap memenuhi kebutuhan fungsional deteksi pelanggaran lalu lintas kampus yang tidak mensyaratkan pemrosesan setiap \textit{frame}---cukup dengan pengambilan sampel pada interval yang memadai untuk menangkap setiap pengendara yang melintas. Stabilitas yang ditunjukkan oleh rentang persentil ke-5 hingga ke-95 (6,02--9,33 FPS) mengindikasikan bahwa 90\% durasi operasi berada dalam koridor kinerja yang konsisten dan dapat diprediksi. Anomali penurunan FPS minimum hingga 1,26 terobservasi hanya pada \textit{frame} awal akibat \textit{cold start} dan alokasi memori inisial. Merujuk pada karakteristik akselerasi GPU untuk arsitektur YOLO \cite{bochkovskiy2020yolov4}, adopsi perangkat grafis kompatibel CUDA diproyeksikan mampu meningkatkan \textit{throughput} sebesar 5--10$\times$ (38--76 FPS), membuka kemungkinan pemrosesan \textit{real-time} penuh.
...
Dekomposisi \textit{latency} per komponen menunjukkan bahwa inferensi YOLOv8 mendominasi 98,6\% total waktu pemrosesan (134,21 ms dari 136,17 ms), dengan pelacakan ByteTrack mengonsumsi 1,91 ms (1,4\%) dan logika pelanggaran hanya 0,05 ms, sebagaimana dirangkum pada Tabel~\ref{tab:latency-komponen}. Median \textit{latency} \textit{end-to-end} tercatat 140,32 ms dengan P95 pada 166,24 ms dan P99 pada 240,43 ms. Proporsi kontribusi \textit{latency} per komponen dan distribusi \textit{latency} total divisualisasikan masing-masing pada Gambar~\ref{fig:latency-breakdown} dan Gambar~\ref{fig:latency-histogram}.
...
\hline
Inferensi YOLOv8         & 134,21 & 138,07 & 163,52 & 234,10 \\
Pelacakan ByteTrack      &   1,91 &   1,87 &   4,00 &   5,42 \\
Logika pelanggaran       &   0,05 &   0,04 &   0,09 &    --- \\
\hline
\textbf{Total pipeline}  & 136,17 & 140,32 & 166,24 & 240,43 \\
\hline
\end{tabular}
\end{table}
...
Dominasi inferensi YOLOv8 (98,6\%) sebagai \textit{bottleneck} komputasi konsisten dengan karakteristik arsitektur \textit{deep neural network} yang menuntut operasi matriks intensif pada tahap \textit{forward pass} \cite{jocher2023ultralytics}. Efisiensi komputasi ByteTrack yang hanya memerlukan 1,91 ms mendemonstrasikan keunggulan algoritma pencocokan berbasis IoU dan prediksi Kalman filter dengan kompleksitas $O(nm)$, di mana $n$ dan $m$ masing-masing merupakan jumlah \textit{track} aktif dan deteksi baru \cite{zhang2022bytetrack}. Logika validasi pelanggaran yang bersifat \textit{negligible} (0,05 ms) merupakan konsekuensi dari desain berbasis operasi \textit{lookup} pada struktur data \textit{dictionary} dengan kompleksitas $O(1)$. Median \textit{latency} 140,32 ms yang secara substansial berada di bawah ambang batas 1 detik memenuhi persyaratan \textit{near real-time}. Perbedaan yang moderat antara P95 (166,24 ms) dan P99 (240,43 ms) mengindikasikan \textit{latency spike} intermiten yang kemungkinan disebabkan oleh \textit{garbage collection} Python atau \textit{context switching} pada level sistem operasi, suatu fenomena yang lazim pada runtime interpreter \cite{belay2012dune}.
...
Pipeline menghasilkan 57 \textit{event} pelanggaran terkonfirmasi pada konfigurasi $N = 3$, dengan \textit{confidence} rata-rata 0,763 dan rentang distribusi 0,541--0,895. Setiap \textit{event} direpresentasikan sebagai objek \texttt{ViolationEvent} dengan atribut komprehensif meliputi \texttt{event\_id} (UUID), \texttt{track\_id}, \texttt{timestamp} (UTC), \texttt{camera\_id}, \texttt{violation\_type}, \texttt{confidence}, \texttt{bbox}, \texttt{frame\_number}, dan \texttt{processing\_latency\_ms}. Sampel representatif dari \textit{event} pelanggaran yang dihasilkan disajikan pada Tabel~\ref{tab:sampel-event}.
...
18  &   78 & 0,842 & gate\_utama\_01 & (701, 207, 748, 285) \\
82  &  745 & 0,891 & gate\_utama\_01 & (994, 298, 1073, 537) \\
170 & 2351 & 0,876 & gate\_utama\_01 & (575, 214, 621, 310) \\
\hline
\end{tabular}
\end{table}
...
Kapasitas \textit{throughput} 3.275 pesan/detik secara substansial melampaui kebutuhan operasional sistem satu kamera yang hanya menghasilkan $\approx$7,6 \textit{event}/detik, mengindikasikan skalabilitas infrastruktur Kafka hingga $\approx$430 kamera simultan tanpa modifikasi konfigurasi \cite{kreps2011kafka}. Optimasi \textit{producer} melalui kompresi \textit{Snappy}, \textit{batch.num.messages} = 100, dan \textit{linger.ms} = 5 ms memaksimalkan efisiensi \textit{micro-batching} tanpa mengorbankan \textit{latency} pengiriman. Parameter \texttt{acks=all} menjamin durabilitas pesan meskipun menambahkan overhead marginal pada waktu \textit{flush} (142,44 ms)---suatu konfigurasi yang merepresentasikan kompromi optimal antara \textit{throughput}, \textit{latency}, dan reliabilitas sebagaimana direkomendasikan untuk sistem \textit{event-driven} berskala produksi \cite{narkhede2017kafka}. \textit{Latency} per pesan yang sangat rendah (median 0,014 ms) mengonfirmasi bahwa overhead transmisi Kafka secara efektif \textit{negligible} terhadap \textit{latency} keseluruhan pipeline.
...
Konsistensi pelacakan & Identitas persisten & 134 \textit{track} unik & \checkmark \\
\textit{Threshold} temporal & Reduksi \textit{false positive} & $N=3$ optimal, +3,7\% \textit{confidence} & \checkmark \\
\textit{Latency} pipeline & $<$ 1 detik & 136,17 ms & \checkmark \\
